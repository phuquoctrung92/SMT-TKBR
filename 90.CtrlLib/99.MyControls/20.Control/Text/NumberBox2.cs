...
namespace CtrlLib.MyControls
{
    ...
    public class NumberBox : BaseTextBox
    {
        ...
        /// <summary> 負数入力を許可するか </summary>
        private bool m_allow_negative_value = true;
        /// <summary> 整数部最大桁数 </summary>
        private int m_integer_length = 7;
        /// <summary> 小数部最大桁数 </summary>
        private int m_decimal_length = 0;

        private bool m_is_display_with_format = true;
        ...
        [model.DefaultValue(true)]
        public bool DisplayWithFormat
        {
            get
            {
                return m_is_display_with_format;
            }
            set
            {
                m_is_display_with_format = value;
            }
        }
                    ...
                    case var case2 when case2 == Strings.Asc(ConstLib.STR_COMMA):        // カンマ(区切り文字)
                        {
                            ...
                            if (decimalPos == 0)
                            {
                                ...
                                if (m_integer_length > 0)
                                {
                                    ...
                                    if (wkStr.Replace(ConstLib.STR_HYPHEN, string.Empty).Length >= m_integer_length && wkPos >= wkStr.Length)
                                    {
                                        e.Handled = true;
                                        return;
                                    }
                            ...
                            else
                            {
                                ...
                                {
                                    // 整数部が最大桁入力されている場合
                                    if (wkPos >= integerValue.Length)
                                    {
                                        e.Handled = true;
                                        return;
                                    }
                                    if (integerValue.Replace(ConstLib.STR_HYPHEN, string.Empty).Length >= m_integer_length && wkPos >= integerValue.Length)
                                    {
                                        e.Handled = true;
                                        return;
                                    }
                    ...
                    default:
                        {
                            ...
                            // 全角が含まれる場合はエラー
                            if (!ClsCheck.IsNarrow(Conversions.ToString(e.KeyChar)))
                            {
                                e.Handled = true;
                                return;
                            }
                            ...
                            if (!string.IsNullOrEmpty(wkStr))
                            {
                                ...
                                if (m_integer_length > 0)
                                {
                                    ...
                                    {
                                        if (Strings.Replace(integerValue, ConstLib.STR_HYPHEN, string.Empty).Length > m_integer_length)
                                        {
                                            e.Handled = true;
                                            return;
                                        }
...

        {
            ...
            if (e.KeyData == Keys.Enter)
            {
                ...
                if (DecimalLength == 0)
                {
                    RepValue = IntegerLength + "桁 以内";
                }
                else
                {
                    RepValue = " 整数 " + IntegerLength + "桁," + " 小数 " + DecimalLength + "桁" + " 以内";
...

        {
            ...
            if (m_integer_length > 0)
            {
                ...
                {
                    // '区切り文字を除いた桁数で判定
                    if (Strings.Replace(integerValue.Replace(ConstLib.STR_COMMA, string.Empty), ConstLib.STR_HYPHEN, string.Empty).Length > m_integer_length)
                    {
                        e.Handled = true;
                        return;
                    }
...

        {
            ...
            try
            {
                ...
                if (m_integer_length > 0)
                {
                    ...
                    // マイナス入力があるので絶対値で判定
                    if (Conversion.Fix(Math.Abs(textValue)).ToString().Length > m_integer_length)
                    {
                ...
                {
                    if (textValue.ToString().Substring(decimalPos).Length > m_decimal_length)
                    {
                        SetLastError(ErrorNumbers.IsLength);
                        return (int)LastErrorNumber;
                    }
